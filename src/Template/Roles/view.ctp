...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('List Finances'), ['controller' => 'Finances', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Finance'), ['controller' => 'Finances', 'action' => 'add']) ?> </li>
...
<div class="roles view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        ...
        <tr>
            <th scope="row"><?= __('Descption') ?></th>
            <td><?= h($role->descption) ?></td>
        </tr>
        <tr>
    ...
    <div class="related">
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($role->finances as $finances): ?>
            <tr>
                <td><?= h($finances->id) ?></td>
                <td><?= h($finances->user_id) ?></td>
                <td><?= h($finances->department_id) ?></td>
                <td><?= h($finances->role_id) ?></td>
                <td><?= h($finances->amount) ?></td>
                <td><?= h($finances->reason) ?></td>
                <td><?= h($finances->receipt) ?></td>
                <td><?= h($finances->state) ?></td>
