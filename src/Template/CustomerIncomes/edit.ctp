...
<nav class="large-2 medium-3 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('删除'),
                ['action' => 'delete', $customerIncome->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $customerIncome->id)]
            )
        ?></li>
...
<div class="finances form large-10 medium-9 columns content">
    ...
    <fieldset>
        <legend><?= __('编辑收益') ?></legend>
        <?php
            echo $this->Form->control('user_id', ['type' => 'hidden', 'value' => $this->request->session()->read('Auth')['User']['id']]);
            echo $this->Form->control('customer_id', ['type' => 'hidden', 'value' => $customerIncome->customer_id]);
